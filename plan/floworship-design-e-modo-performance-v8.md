# Floworship — Design System, Plataformas e Modo Performance (v8)

## Especificação para Desenvolvimento

> **Nota de versão (v8):** adicionada a seção 10 — Catálogo de Músicas e Repertório do Culto — separando o catálogo oficial (músicas com status de preparo) do repertório específico de cada domingo, definido pelo Ministro de Louvor escalado depois que a escala é aprovada. Próximos Passos Técnicos passa a ser a seção 11.
...
### 4.5 Modo Estudo — Afinador e Metrônomo (Mobile, exclusivo)
...
**Considerações específicas do Capacitor (WebView):**

1. **Permissões nativas obrigatórias** para o afinador funcionar: `NSMicrophoneUsageDescription` no `Info.plist` (iOS) e `RECORD_AUDIO` no `AndroidManifest.xml` (Android). Em alguns dispositivos o prompt de permissão do `getUserMedia` não dispara sozinho dentro da WebView — recomenda-se usar um plugin de permissão nativo (ex: `@capacitor/microphone` ou equivalente) para solicitar a permissão explicitamente antes de iniciar a captura.
2. **Suspensão de áudio em background/tela bloqueada:** WebViews em iOS tendem a suspender o `AudioContext` quando o app perde foco. Isso é crítico não só para o afinador/metrônomo, mas também para o **Modo Operador ao vivo** (seção 4.1) caso a tela apague durante a execução — recomenda-se testar esse comportamento cedo e, se necessário, usar wake-lock (`@capacitor/keep-awake` ou similar) durante sessões ativas.
3. Ambas as ferramentas (afinador e metrônomo) compartilham o mesmo `AudioContext` — vale centralizar sua criação/gestão em um serviço único (`AudioEngineProvider`) para evitar múltiplas instâncias concorrendo pelo hardware de áudio.
...
1. **Disparo mensal** (ex: dia 20 do mês anterior, configurável) — sistema cria o `monthly_schedule_cycle`, calcula os domingos do mês-alvo e dispara a pergunta de disponibilidade **via WhatsApp** (ver 9.8) para todo o quadro ativo:
...
6. **Publicação** — ao aprovar, cada `service_schedule` vira `"publicada"` e cada músico escalado recebe a confirmação da sua vaga **via WhatsApp**.
...
**Canal de notificação: WhatsApp como canal primário.** Diferente da decisão inicial (push via PWA), o fluxo de disponibilidade/confirmação/substituição roda **pelo WhatsApp**, não pelo app — ver arquitetura completa em 9.8. Isso elimina a desculpa de "não vi o app" e usa o canal onde as pessoas realmente respondem rápido no Brasil.
...
### 9.8 Canal de Notificação — WhatsApp como Canal Primário

**Provedor: Meta Cloud API (oficial), não simuladores tipo Z-API.** Existem duas famílias de solução pra WhatsApp no Brasil: (1) a **API oficial da Meta** (Cloud API, direto ou via camada de gestão como Twilio) e (2) **simuladores não-oficiais** (Z-API e afins), que automatizam o WhatsApp Web por trás e são mais baratos/rápidos de configurar, mas violam os termos de uso do WhatsApp e carregam risco real de **banimento do número**. Como esse canal vira a espinha dorsal do motor de escalas (não um recurso acessório), o risco de um número banido travar a geração de escalas do mês inteiro é grande demais — recomendo a **API oficial da Meta** desde o início, mesmo com o processo de verificação de negócio e aprovação de templates sendo um pouco mais burocrático.

**Janela de 24h e templates aprovados:** a política da Meta só permite texto livre dentro de 24h após a última mensagem *do usuário*. Como o motor de escalas sempre **inicia** a conversa (pergunta de disponibilidade, convite de substituição), essas mensagens precisam usar **templates pré-aprovados** pela Meta — texto fixo com variáveis (`{{nome}}`, `{{data}}`, `{{funcao}}`) e botões predefinidos. A aprovação de um template leva de horas a poucos dias, então todos os templates do motor de escalas precisam estar aprovados e testados **antes** do lançamento — isso é dependência de cronograma, não só técnica.

**Botões interativos em vez de texto livre:** usar *Reply Buttons* (até 3 por mensagem) pra respostas binárias como "Disponível" / "Não disponível", e *List Messages* quando a pergunta cobre vários domingos de uma vez. Isso evita depender de interpretação de texto livre (NLP) — desnecessário aqui, já que toda resposta esperada é estruturada.

**Templates necessários (submeter e aprovar na Meta Business Manager antes do lançamento):**
1. `disponibilidade_mensal` — pergunta de disponibilidade por domingo, em lista interativa
2. `escala_confirmada` — aviso de escala publicada ("Você está escalado(a) para {{data}} como {{funcao}}"), com botão opcional "Ver escala completa" (abre o app)
3. `substituicao_urgente` — convite de substituição de última hora, com prazo de resposta e botões "Aceito" / "Não posso"
4. `lembrete_disponibilidade` — lembrete pra quem não respondeu (D-3/D-1 do prazo)

**Arquitetura do fluxo:**
```
Backend (Fastify) dispara mensagem via Meta Cloud API (template aprovado + botões)
  → usuário responde no WhatsApp
  → Meta envia webhook ao backend com a resposta estruturada (button_reply.id)
  → backend atualiza availability_response / service_assignment
  → segue o fluxo normal já definido (9.3, 9.6)
```
...
musician {
  ...
  whatsapp_phone: string        // formato E.164, ex: "+5544999998888"
  whatsapp_opt_in: boolean      // consentimento explícito, coletado no aceite do convite
}
...
whatsapp_message_log {
  ...
  response_payload?             // button_reply recebido, se houver
}
```

**Opt-in obrigatório:** a Meta exige consentimento explícito antes do primeiro contato de negócio. Isso entra no fluxo de convite (seção 7.1): ao aceitar o convite, o músico confirma o número de WhatsApp e consente em receber as notificações do ministério — sem isso, o número não pode ser incluído no motor de escalas.

**Papel do app com WhatsApp como canal primário:** o app (PWA) deixa de ser o lugar de confirmação/resposta e passa a ser o lugar de **consulta e execução** — ver a escala detalhada, estudar repertório (Modo Estudo), e sobretudo o Modo Operador/Letra/Cifra ao vivo, que continua exclusivo do app (WhatsApp não serve pra isso). O push notification via PWA (seção 8.2) permanece útil pra avisos gerais e pro card de "sessão ao vivo" (seção 6.2), mas deixa de ser crítico pro motor de escalas em si.
...
Escala do domingo aprovada/publicada (seção 9.3, passo 5-6)
  ...
  → banda escalada naquele domingo recebe o repertório via WhatsApp
...
- [x] ~~Decidir canal de notificação do motor de escalas~~ — **WhatsApp (Meta Cloud API oficial) como canal primário**, com templates aprovados e botões interativos; app fica com papel de consulta/execução, não confirmação (ver 9.8)
- [ ] Criar conta Business Manager na Meta e submeter verificação de negócio (pré-requisito pra usar a Cloud API — iniciar cedo, processo pode levar dias)
- [ ] Redigir e submeter os 4 templates de mensagem (`disponibilidade_mensal`, `escala_confirmada`, `substituicao_urgente`, `lembrete_disponibilidade`) para aprovação da Meta antes de qualquer teste de ponta a ponta
- [ ] Implementar endpoint de webhook para receber respostas do WhatsApp (`button_reply`) e validar assinatura da Meta na requisição
- [ ] Adicionar campo de WhatsApp + opt-in explícito ao fluxo de aceite de convite (seção 7.1)
...
- [ ] Redigir e submeter o template `repertorio_definido` junto com os demais da seção 9.8
