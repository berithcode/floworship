...
export async function songsRoutes(fastify: FastifyInstance) {
  ...
  // Hard delete song (permanently remove from database)
  fastify.delete<{ Params: { id: string } }>('/songs/:id/permanent', async (request: any, reply: any) => {
    const user = getUser(request);
    if (!user?.ministryId) {
      return reply.status(400).send({ error: 'Ministry required' });
    }
    ...
    const cueSheet = await prisma.songCueSheet.findFirst({ where: { songId: song.id } });
    if (cueSheet) {
      await prisma.cueBlock.deleteMany({ where: { cueSheetId: cueSheet.id } });
      await prisma.songCueSheet.delete({ where: { id: cueSheet.id } });
    }

    await prisma.serviceRepertoireItem.deleteMany({ where: { songId: song.id } });
