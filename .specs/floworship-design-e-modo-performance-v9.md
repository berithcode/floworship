# Floworship — Design System, Plataformas e Modo Performance (v9)

## Especificação para Desenvolvimento

> **Nota de versão (v9):** WhatsApp pausado como canal de notificação (problemas reais com API não-oficial + API oficial da Meta inviável no momento, seção 9.8). Canal primário passa a ser **Telegram Bot API + push nativo via PWA**, rodando em paralelo nessa fase de testes. Campos e modelo de WhatsApp seguem no schema sem uso, não foram removidos — decisão é reversível.
